...
- 右侧信息栏按钮与胶卷栏按钮应视为同一套控件家族：材质、颜色、描边、高光、阴影、图标权重和交互反馈保持一致；只允许因挂载边缘不同而保留几何比例、定位方式和状态偏移上的差异。
...
7. 按钮对齐规则建议明确记录：信息栏按钮挂在右侧壳层边缘，胶卷栏按钮挂在底部壳层上边缘；两者不是完全同几何实现，但视觉目标必须是“同材质、同层级、同图标权重”，避免一个像系统控件、另一个像单独拼出来的浮动按钮。
